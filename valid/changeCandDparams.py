...
if __name__ == "__main__":
    ...
    D_values = [10, 100, 10000, 162410]

    for file_name in input_files:
        # Un CSV par forme
        csv_path = os.path.join(output_dir, f"results_summary_CandD_{file_name}.csv")
        csv_file = open(csv_path, "w", newline="")
        csv_writer = csv.writer(csv_file)

        # En-tête dynamique : Shape, C, D, Wins, Losses, Draws, Errors, run1...run10
        header = ["Shape", "C", "D", "Wins", "Losses", "Draws", "Errors"] + [f"run{i+1}" for i in range(10)]
        csv_writer.writerow(header)

        log_path = os.path.join(output_dir, f"saveResultsCandD_{file_name}.txt")
            ...
            for C in C_values:
                for D in D_values:
                    ...
                    for i in range(10):
                        ...
                        durations.append(round(run_time, 2))
                        ...
                        print(f"Run {i+1}: {results[-1]} - {durations[-1]:.2f}s")
                        log.write(f"Run {i+1}: {results[-1]} - {durations[-1]:.2f}s\n")
                    ...
                    log.write(f"Summary: W={summary['W']}, L={summary['L']}, D={summary['D']}, E={summary['E']}\n")
                    log.flush()

                    # Ligne CSV : Shape, C, D, W, L, D, E, run1...run10
                    csv_writer.writerow([
                                            file_name,
                                            C,
                                            D,
